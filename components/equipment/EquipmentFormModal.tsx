  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        vendorPhone2: initialData.vendorPhone2 || '',
        note: initialData.note || ''
      });
    } else {
      setFormData({ name: '', category: '', count: 1, vendorName: '', vendorPhone: '', vendorPhone2: '', note: '' });
    }
  }, [initialData]);
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-slate-500 mb-1">업체 대표 전화</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-xs text-slate-500 mb-1">담당자 직통 전화</label>
                  <input
                    type="tel"
                    value={formData.vendorPhone2}
                    onChange={e => setFormData({...formData, vendorPhone2: e.target.value})}
                    className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
