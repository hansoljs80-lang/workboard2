...
import { BedDouble, Settings, PlayCircle, Info } from 'lucide-react';
...
const BedManager: React.FC<BedManagerProps> = ({ staff, tasks, settings, onRefresh, onNavigateToBoard }) => {
  ...
  return (
    <div className="p-4 md:p-6 w-full h-full overflow-y-auto pb-24 relative bg-slate-50 dark:bg-slate-950">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
               총 {config.count}개 • {config.interval}일 주기 (매주 {weekDayName}요일) 
               <span className="hidden md:inline mx-2 text-slate-300 dark:text-slate-600">|</span>
               <br className="md:hidden" />
               <span className="font-bold text-blue-600 dark:text-blue-400">다음 예정: {nextDateDisplay}</span>
            </p>
          </div>
        </div>
      ...
      {/* Usage Tip Banner */}
      <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800 flex items-start gap-3">
         ...
         <div className="text-sm text-blue-800 dark:text-blue-200">
            ...
            <p>
              ...
              그러면 다음번 <strong>정기 교체 업무 생성</strong> 시 해당 배드는 <strong>'최근 교체됨'</strong>으로 표시되어 중복 교체를 방지할 수 있습니다.<br/>
              이미 생성된 <strong>정기 교체 업무</strong>가 있다면 자동으로 <strong>체크(완료)</strong> 처리됩니다.
            </p>
         </div>
      </div>
      ...
      >
        ...
        <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-${config.cols} gap-4 bed-grid`}>
          {beds.map(bed => (
            <BedCard 
              key={bed.id} 
              bed={bed}
