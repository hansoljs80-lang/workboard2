...
interface BedChangeModalProps {
  ...
  bedName: string;
  staff: Staff[];
  initialSelection?: string[]; // For editing
  onClose: () => void;
...
const BedChangeModal: React.FC<BedChangeModalProps> = ({ 
  ...
  // Filter staff to show only active ones, OR staff who are already selected (even if inactive)
  const visibleStaff = useMemo(() => {
    return staff.filter(s => s.isActive !== false || initialSelection.includes(s.id));
  }, [staff, initialSelection]);
  ...
  const handleConfirm = () => {
    ...
    onClose();
    // Don't clear immediately to prevent UI flicker, handled by useEffect on reopen
  };
      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-2xl shadow-xl overflow-hidden animate-fade-in-up border border-slate-100 dark:border-slate-700">
        <div className="p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
             <span className="font-bold text-slate-700 dark:text-slate-300">{bedName}</span> 
             {isEditMode ? ' 작업을 수행한 직원을 수정합니다.' : ' 커버를 교체한 직원을 선택하세요.'}
          </p>
        </div>
