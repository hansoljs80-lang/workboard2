...
import { Monitor, Search, Plus, Filter, AlertCircle, Copy } from 'lucide-react';
import { Equipment } from '../types';
import { fetchEquipments, deleteEquipment, updateEquipment } from '../services/equipmentService';
...
const EquipmentManager: React.FC = () => {
  ...
  const handleCountChange = async (id: string, newCount: number) => {
    ...
    // Silent API Call
    try {
      await updateEquipment(id, { count: newCount });
    } catch (e) {
      console.error(e);
      // Revert on failure
      loadData();
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 p-4 md:p-6 pb-6 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 shrink-0">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-cyan-600 text-white rounded-xl font-bold shadow-md hover:bg-cyan-700 transition-all active:scale-95 self-start md:self-auto"
        >
          <Plus size={20} />
