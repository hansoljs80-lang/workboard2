...
import { 
  ...
  RefreshCw, Trophy, Crown, AlertCircle 
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 overflow-hidden">
            ...
            <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={() => {
                     ...
                     const s = new Date(currentDate); // trigger effect
                     setCurrentDate(s);
                  }}
                  className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-500 hover:text-blue-500 transition-colors"
                >
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
               ...
               <div className="space-y-3">
                  {stats.map((s, idx) => (
                     <div key={s.id} className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-slate-600 dark:text-slate-400 w-8 text-right">{s.count}회</span>
                     </div>
                  ))}
               </div>
            )}
         ...
         {/* RIGHT: Log List */}
         <div className="flex-1 overflow-y-auto custom-scrollbar p-4 bg-slate-50/50 dark:bg-black/20">
            {loading ? (
