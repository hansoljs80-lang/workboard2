...
import { ChangingRoomConfig } from '../../types';
import { X, Save, Sun, Coffee, Eye } from 'lucide-react';
import ConfigListEditor from '../common/ConfigListEditor';

interface ChangingRoomConfigModalProps {
  config: ChangingRoomConfig;
  onSave: (config: ChangingRoomConfig) => void;
  onClose: () => void;
}

const ChangingRoomConfigModal: React.FC<ChangingRoomConfigModalProps> = ({ config, onSave, onClose }) => {
  const [localConfig, setLocalConfig] = useState<ChangingRoomConfig>(config);
  const [activeTab, setActiveTab] = useState<'morning' | 'lunch' | 'adhoc'>('morning');

  const getListKey = () => {
    switch (activeTab) {
      case 'morning': return 'morningItems';
      case 'lunch': return 'lunchItems';
      case 'adhoc': return 'adhocItems';
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 shrink-0">
           <h3 className="font-bold text-slate-800 dark:text-slate-100">탈의실 점검 목록 설정</h3>
        ...
        <div className="flex-1 overflow-hidden min-h-[300px]">
          <ConfigListEditor 
            ...
            placeholder={`${activeTab === 'morning' ? '아침' : activeTab === 'lunch' ? '점심' : '수시'} 점검 항목 추가...`}
          />
        </div>
...
export default ChangingRoomConfigModal;