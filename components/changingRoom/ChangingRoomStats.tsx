...
import { BarChart3, Trophy, Sun, Coffee, Eye } from 'lucide-react';
...
interface ChangingRoomStatsProps {
  stats: StatsItem[];
  shiftLeaders: {
    MORNING: { name: string; count: number } | null;
    LUNCH: { name: string; count: number } | null;
    ADHOC: { name: string; count: number } | null;
  };
  loading: boolean;
}

const ChangingRoomStats: React.FC<ChangingRoomStatsProps> = ({ stats, shiftLeaders, loading }) => {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm h-full flex flex-col">
      <div className="flex items-center gap-2 mb-4 pb-2 border-b border-slate-100 dark:border-slate-800 shrink-0">
         ...
         <h3 className="font-bold text-slate-800 dark:text-slate-100">이달의 탈의실 관리왕</h3>
      </div>
           ...
           <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2 mb-2">
                 ...
                 <div className="bg-orange-50 dark:bg-orange-900/10 p-2 rounded-lg border border-orange-100 dark:border-orange-800 flex flex-col items-center text-center">
                    <div className="p-1 bg-white dark:bg-slate-800 rounded-full mb-1 shadow-sm text-orange-500">
                      <Coffee size={14} />
                    </div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mb-1">점심</span>
                    {shiftLeaders.LUNCH ? (
                      <>
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate w-full">{shiftLeaders.LUNCH.name}</span>
                        <span className="text-[10px] text-orange-600 font-bold">{shiftLeaders.LUNCH.count}회</span>
                      </>
                    ) : <span className="text-[10px] text-slate-400">-</span>}
                 </div>

                 <div className="bg-teal-50 dark:bg-teal-900/10 p-2 rounded-lg border border-teal-100 dark:border-teal-800 flex flex-col items-center text-center">
                    <div className="p-1 bg-white dark:bg-slate-800 rounded-full mb-1 shadow-sm text-teal-500">
                      <Eye size={14} />
                    </div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mb-1">수시</span>
                    {shiftLeaders.ADHOC ? (
                      <>
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate w-full">{shiftLeaders.ADHOC.name}</span>
                        <span className="text-[10px] text-teal-600 font-bold">{shiftLeaders.ADHOC.count}회</span>
                      </>
                    ) : <span className="text-[10px] text-slate-400">-</span>}
                 </div>
              </div>
                  ...
                  {stats.map((item, index) => (
                    <div key={item.id} className="flex items-center gap-3 animate-fade-in" style={{ animationDelay: `${index * 50}ms` }}>
                      ...
                      <div className="flex flex-col items-end gap-0.5 w-24">
                          <span className="text-sm font-bold text-teal-600 dark:text-teal-400">{item.count}회</span>
...
export default ChangingRoomStats;